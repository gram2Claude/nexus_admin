...
Положи `WORKFLOW.md` в `work_directory/` нового репозитория, открой Claude Code, скажи
«прочитай `work_directory/WORKFLOW.md` и приступай» — и Claude проведёт от пустой папки до ship.
...
mkdir work_directory/tests
mkdir work_directory/tests/fixtures
...
│   ├── WORKFLOW.md    — этот файл (стартовая инструкция Claude для проекта)
...
├── TODOS.md       — deferred work из ревью (в КОРНЕ — этого ждут gstack-скиллы)
...
2. Сохрани findings в `work_directory/tests/fixtures/<SOURCE>_RECON.md` с разделами:
...
- **Задача с внешним источником:** recon (Phase 3) ДО кода вскрыл, что целевой сайт закрыт anti-bot (JS-challenge) → план развернули v1→v2→v3 (например, на RSS) ещё до написания кода. Экономия дней работы на неверной архитектуре. Findings — в `work_directory/tests/fixtures/<SOURCE>_RECON.md`.
